// app.js - multi-page site logic for Deenice Finds
document.addEventListener('DOMContentLoaded', function(){
  const PRODUCTS = [
    {id:'p1',slug:'chromeheart-beanies',title:'Chromeheart Beanies',price:1900,desc:'Warm ribbed beanie with embroidered logo.',images:['https://i.postimg.cc/441XX552/O1-CN01-Sj-Tb-Ky2-G1tx7oc-Nfp-2213981258956-0-cib.jpg'],colors:[{name:'Black',hex:'#000'},{name:'Grey',hex:'#9CA3AF'},{name:'Beige',hex:'#D9CAB3'}]},
    ...
    {id:'p3',slug:'chromeheart-sunglasses',title:'ChromeHeart Sunglasses',price:2600,desc:'UV protection sunglasses with metallic trim.',images:['https://i.postimg.cc/PJfsCg4c/O1-CN01-E4vv4r1d4-PHxocq-S4-2214783223682-0-cib.jpg'],colors:[{name:'Black',hex:'#000'},{name:'Gold',hex:'#D4AF37'},{name:'Brown',hex:'#8B5E3C'}]},
      ...
      card.innerHTML = `
        ...
        <div class="card-body">
          <div class="row-between">
            ...
            <div class="price">${p.currency||'KES'} ${formatPrice(p.price)}</div>
          </div>
  ...
  if(detail){
      ...
      <div class="product-detail">
        <div style="display:flex;gap:18px;flex-wrap:wrap">
          <div class="product-gallery" style="flex:1 1 520px">
            <div class="product-main"><img id="mainImg" src="${p.images[0]}" style="width:100%;border-radius:12px;object-fit:cover" loading="lazy"></div>
    ...
    p.images.forEach(img=>{
      const t = document.createElement('img'); t.src=img; t.style.width='100px'; t.style.height='70px'; t.style.objectFit='cover'; t.style.borderRadius='8px'; t.loading='lazy';
      t.addEventListener('click', ()=> document.getElementById('mainImg').src = img);
      thumbRow.appendChild(t);
    });
    const prodColors = document.getElementById('prodColors');
    p.colors.forEach((c,i)=>{
      const s = document.createElement('span'); s.className='color-swatch' + (i===0? ' selected':'');
      s.style.background = c.hex || '#ccc'; s.title=c.name;
      s.addEventListener('click', ()=>{ document.querySelectorAll('#prodColors .color-swatch').forEach(x=>x.classList.remove('selected')); s.classList.add('selected'); });
    ...
    document.getElementById('buyBtn').addEventListener('click', ()=>{
      ...
      const text = `Hi! I'm interested in ${p.title} (${color}) - ${p.currency||'KES'} ${formatPrice(p.price)}`;
  ...
  if(blogsGrid){
    const BLOGS = [
      {slug:'how-to-take-good-pictures',title:'How to Take Stunning Pictures Using a Magnetic Selfie Display',excerpt:'Quick tips on lighting, framing and editing with a magnetic selfie display.',image:'https://i.postimg.cc/PJfsCg4c/O1-CN01-E4vv4r1d4-PHxocq-S4-2214783223682-0-cib.jpg',date:'2025-10-14'},
      {slug:'demo-article',title:'Demo Article: Styling Tips',excerpt:'Short demo article.',image:'https://i.postimg.cc/441XX552/O1-CN01-Sj-Tb-Ky2-G1tx7oc-Nfp-2213981258956-0-cib.jpg',date:'2025-10-10'}
    ];
    BLOGS.forEach(b=>{
      const c = document.createElement('article'); c.className='blog-card';
      c.innerHTML = `<div style="display:flex;gap:12px;align-items:center"><img src="${b.image}" style="width:110px;height:70px;object-fit:cover;border-radius:8px"><div><h3 style="margin:0">${b.title}</h3><p class="muted" style="margin:6px 0">${b.excerpt}</p><a href="blogs/${b.slug}.html">Read more</a></div></div>`;
      blogsGrid.appendChild(c);
    });
  }
  ...
  if(searchInput){
    searchInput.addEventListener('input', function(e){
      ...
      if(!q) return;
      const prodMatches = PRODUCTS.filter(p => p.title.toLowerCase().includes(q) || p.desc.toLowerCase().includes(q)).slice(0,6);
      const list = [...prodMatches];
      list.forEach(item => {
        const li = document.createElement('li');
        li.innerHTML = `<img src="${item.images[0]}" alt="${item.title}"><div style="flex:1"><strong style="display:block">${item.title}</strong><small class="muted">KES ${formatPrice(item.price)}</small></div>`;
  ...
  setTimeout(()=> document.getElementById('page') && document.getElementById('page').classList.add('in'), 80);
});